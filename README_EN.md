...
> **STARHE** = **S**tratification of risk and detection of **H**epatocellular carcinoma by **E**chography (ultrasound).  
> Python/Go extension for the [MEDomics](https://medomicslab.gitbook.io/medomics-docs) platform.

*Version `0.2.0` — Last updated: July 10, 2025*

---

## Overview

The plugin analyzes abdominal ultrasound DICOM cine-clips to screen for hepatocellular carcinoma (HCC). It operates in **two modes**:

| Mode | Description |
|---|---|
| **Standalone** | Self-contained Go server (`go_server/`) + Tkinter prototype. Go launches `pipeline.py` as a subprocess and streams results via SSE. |
...
> **MongoDB port 54017**: MEDomics deliberately uses a non-standard port to avoid conflicts with system MongoDB instances. This port is set in `config.py` AND `go_server/config.go`.
...
Both scripts are **self-contained**: they detect Python 3.13, create the venv if missing, install all dependencies and prepUS, then launch the UI. Only Python 3.13 needs to be installed on the system.

**Windows (PowerShell):**

> **One-time prerequisite**: allow local PowerShell scripts (once per user):
> ```powershell
...
The script detects Python 3.13 on the system (via `py -3.13`, `python3.13`, or `python`), checks that tkinter is available, creates the venv if missing, installs dependencies, then launches the UI.
...
The script `run_tkinter.sh` checks that Python 3.13 and tkinter are present, creates the venv and installs dependencies if missing, installs prepUS, then launches the UI.
...
<details>
<summary>Manual equivalent commands (macOS / Linux)</summary>
...
<details>
<summary>Manual equivalent commands (Windows PowerShell)</summary>
...
Python paths are detected **automatically** by `config.go` from the `go_server/` folder (relative path `../pythonCode/modules/…`). No environment variable is needed if the venv was created in step 1 and the server is launched from `go_server/`.
...
| `STARHE_PYTHON_EXE` | absolute path in `config.go` | Python 3.13 from the venv |
| `STARHE_PYTHON_PATH` | absolute path in `config.go` | Root folder of Python modules |
| `MONGO_URI` | `mongodb://localhost:54017/` | MongoDB URI |
| `MONGO_DB` | `medomics` | Database name |
| `MONGO_COLL` | `starhe_results` | Collection name |

### 3. Deploy to MEDomics (integrated mode)

> This mode is described in detail in the **MEDomics Integration** section below.  
> The plugin is deployed into the MEDomics repository via symlinks and a Go blueprint.

---

## Architecture

### Standalone Mode (self-contained Go server)
...
MEDomics Frontend (React) / Tkinter UI
  ...
  starhe_plugin/pipeline.py  → main orchestrator
        ...
        ├── ai/starhe_detect.py    → STARHE-DETECT (RTMDet persistent subprocess)
...
MEDomics Frontend (Electron / React)
  ...
  pythonCode/modules/starhe_plugin/pipeline.py  → full pipeline (dedicated STARHE venv)
        ...
        └── ... (same modules as standalone mode)
```

**Key difference**: in integrated mode, `run_starhe.py` acts as a bridge between two distinct Python environments:
...
data: {"level":"result","message":"Pipeline complete","data":{...}}
data: [DONE]
```
...
1. **DICOM Loading** — `load_dicom()` with `pydicom force=True` (supports extensionless files).
...
3. **Frame Extraction** — `extract_frames()` returns `(T, H, W)` or `(T, H, W, 3)` as `uint8`.
4. **prepUS Preprocessing** — see dedicated section below.
5. **STARHE-RISK** — C3D inference on the full clip.
6. **STARHE-DETECT** — RTMDet inference frame by frame (with temporal subsampling).
...
prepUS is MEDomics' ultrasound image preprocessor. It is **vendored** in `third_party/prepUS/` to avoid an external dependency.

### What prepUS Does

- Detects and removes static elements from the ultrasound interface (text, rulers, borders) by analyzing temporal pixel variability.
- Crops the US cone to remove black margins.
- Performs inverse scan conversion (backscan): image reconstruction in a 512×512 Cartesian space, which corrects the ultrasound sector distortion.
...
C3D is a 3D (spatiotemporal) convolutional network defined in `ai/models/c3d.py` in pure PyTorch — **no mmaction2/mmcv dependency** at runtime.
...
Input:  (N, 3, 16, 112, 112)  — N clips, 3 channels, 16 frames, 112×112
  ...
  flatten → fc6(4096) → relu → dropout
            fc7(4096) → relu
  head I3DHead: fc_cls(2) → softmax
Output: (N, 2)  — proba [low_risk, high_risk]
```
...
- **10 clips** uniformly sampled across the full duration (`NUM_CLIPS=10`)
...
Display threshold: none applied, the raw [0–1] score is returned.
...
mmdet/mmcv uses compiled C extensions (`mmcv._ext`) and Python 2 frame metadata incompatible with Python 3.13. The adopted solution is an **isolated subprocess** that runs the RTMDet runner in a context where necessary patches are applied.
...
starhe_detect.py (main process)
    ...
    _rtmdet_runner.py (subprocess)
        ...
        │   1. stub mmcv._ext (replaces missing C extension)
        │   2. stub tqdm (optional, avoids an ImportError)
        │   3. patch inspect.getmodule (Python 3.13 / mmengine compat)
        │
        │ loads the RTMDet model (428 MB) ONCE
...
3. Blocking wait for the line `[rtmdet_server] READY` on stdout
4. Any other line = failure → `RuntimeError` with the last 2000 characters of stderr

### Sending a Frame Batch
...
In `pipeline.py`, only 1 frame out of `DETECT_EVERY_N=4` is sent to the model. The 3 intermediate frames inherit the same detections:
...
Defined in `ai/models/_dino_runner.py`. No server mode — each frame launches a separate subprocess (slow, for development only). Selectable via `DETECT_BACKEND = "dino"` in `config.py`.

---

## MEDomics Integration (Standard Plugin)

The STARHE plugin integrates into the MEDomics platform following the "Standard Plugin" pattern (analogous to 3D Slicer extensions). The integration consists of three pieces:

### 1. Go Blueprint (`medomics_integration/starhe_blueprint.go`)

Go file to be copied to `MEDomics/go_server/blueprints/starhe/starhe.go`. It registers two routes:

| Route | Function | Description |
|---|---|---|
| `starhe/analyze/` | `handleAnalyze` | Launches the STARHE pipeline via `Utils.StartPythonScripts()` |
| `starhe/progress/` | `handleProgress` | Returns the current job progress |
...
JSON file at the project root documenting integration elements (routes, paths, commands to add to MEDomics' `main.go`) and standalone configuration.

### Deployment

Deployment into the MEDomics repository is done by:

1. **Copying** the Go blueprint → `MEDomics/go_server/blueprints/starhe/starhe.go`
2. **Symlinks** in `MEDomics/pythonCode/modules/`:
   - `starhe/` → adapter (`run_starhe.py`)
   - `starhe_plugin/` → the full plugin (pipeline, AI, DICOM, DB…)
3. **Patching** `MEDomics/go_server/main.go` (import + `AddHandleFunc()`)
...
Local port `54017` (non-standard, configured in `config.py` and `go_server/config.go`). Each `_get_collection()` call opens a connection with a 3s timeout — no global pool on the Python side (pymongo manages its own pool).
...
- The cache key is the pair `(file_path, analysis_mode)` — one document per file **and per** analysis mode (`original`, `crop`, `backscan`). Sensitive to file moves/renames.
...
find_by_file(file_path, analysis_mode=None)  # → dict | None  (optional mode filter)
...
## Tkinter Prototype UI (`ui/prototype_tkinter.py`)

The prototype is used to validate the pipeline and UX before the React port. It is a single file of approximately 2500 lines.
...
**Results per display mode**: detections and results are stored in mode-indexed dicts (`_detections_by_mode` and `_results_by_mode`, keys: `"backscan"`, `"crop"`, `"original"`). When the user switches between modes (crop/backscan toggle), only the bounding boxes and results for the active mode are displayed. The `_refresh_results_panel()` method updates the Mode, Risk, and Lesions labels accordingly.

**Measurements in mm**: calibration follows this priority order in DICOM metadata:
...
**Playback loop**: the `_tick()` method is called via `self.after(delay_ms, self._tick)`. The delay is calculated from `FrameTime` DICOM (in ms) divided by `_speed_mult`. For speeds ≥1, frames are skipped (`_skip_n`) instead of reducing the delay (limited to ~15ms by `after`).

**go_print on UI side**: at initialization, `set_log_sink(lambda level, msg: self._append_log(msg))` redirects all messages to the interface console. The sink is reset to `None` on close.

**Zoom and pan**: all canvas coordinates are recalculated on each `_refresh_canvas()` by applying the affine transform `(x * zoom + pan_x, y * zoom + pan_y)`. Images are resized via `PIL.Image.resize` with `LANCZOS`.

**Anonymization on import**: original values are saved in `original_sensitive` (list of tuples `(tag_name, value)`) before anonymization. They are displayed in red in the metadata panel. Anonymized values are in `kept_metadata`.

### Keyboard Shortcuts

A `_kb_guard()` guard checks that the focus is not on a text widget (`tk.Entry`, `tk.Text`, `scrolledtext.ScrolledText`) before executing the shortcut — avoids interference with user input.

---

## Python 3.13 Compatibility

Python 3.13 introduced several breaking changes with mmcv/mmdet. Here are the patches applied in `_rtmdet_runner.py` **before any mmcv import**:

### 1. `mmcv._ext` Stub

mmcv attempts to import a compiled C extension `mmcv._ext` (`NMSop`, etc.). This extension does not exist in recent versions or with incompatible builds. The stub replaces the module with a Python object whose every attribute raises a `RuntimeError` only when called:
...
RTMDet inference does not actually use the NMS functions from the extension (PyTorch provides its own).
...
tqdm is not in mmdet dependencies. If absent, mmdet raises an `ImportError` on import. The stub injects a minimal module where `tqdm.tqdm(iterable)` returns the iterable as-is.
...
├── setup.sh                          # macOS/Linux venv + dependency setup (without UI)
├── setup.ps1                         # Windows venv + dependency setup (without UI)
├── plugin.json                       # Plugin manifest (standalone config + MEDomics integration)
├── README.md                         # Developer documentation (French)
├── README_EN.md                      # Developer documentation (English — this file)
├── READMEUtilisateur.md              # Tkinter UI user guide (French)
├── READMEUtilisateur_EN.md           # Tkinter UI user guide (English)
├── TODOLIST.md                       # Task log / roadmap (French)
├── TODOLIST_EN.md                    # Task log / roadmap (English)
├── MEDomicsLab_LOGO.png              # Logo displayed in the UI
│
├── go_server/                        # Self-contained Go server (standalone mode)
│   ├── main.go                       # HTTP routing + MongoDB init
│   ├── config.go                     # Environment variables with defaults
│   └── handlers.go                   # REST handlers + SSE streaming
│
├── medomics_integration/             # Files intended for the MEDomics repository
│   └── starhe_blueprint.go           # Go blueprint (starhe/analyze, starhe/progress routes)
│
├── third_party/
...
└── pythonCode/modules/
    ...
    │
    └── starhe_plugin/                # Full STARHE plugin
        │
        ...
        │   ├── starhe_detect.py      # RTMDet/DINO wrapper: persistent subprocess
        │   └── models/
        ...
        │
        ├── ui/
        │   └── prototype_tkinter.py  # Prototype UI (~2500 lines)
        │
        └── utils/
...
```python
DATA_DIR   = os.environ.get("STARHE_DATA_DIR", os.path.join(PROJECT_ROOT, "data"))  # DICOM files folder
...
These variables allow sharing the same MongoDB instance between the standalone plugin and the MEDomics platform without code changes.
