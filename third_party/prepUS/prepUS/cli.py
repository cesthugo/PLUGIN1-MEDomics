...
def removeLayout(input_dir: str, output_dir: str,
    ...
    output_dir: path to generate one dir per video with extracted stuff
    """
...
def scanConversion(IRSC: str, output_file: str, dc: float, rc: float, theta_c: float,
                   xoffset: int, yoffset: int,
                   scan_width: int, scan_height: int,
                   backscan_width: int = 512, backscan_height: int = 512) -> None:
    ...
    with Progress() as progress:
        ...
        for i in range(f):
            frame = dsc_image_vectorized(v[i], dc, rc, theta_c, yoffset, xoffset,
                                         scan_width, scan_height, backscan_width, backscan_height)
...
def removeLayoutFile(input_file: str, output_dir: str,
    ...
    if thresh == -1:
        # auto threshold based on mean values
    ...
    if back_scan_conversion:
        ...
        if params:
            ...
            rich.print("[red] find_linear_fov failed!")
        ...
        mask_valid = pre_dsc_image_vectorized(y_cropped[0], dc, rc, theta_c,
                                              yoffset, xoffset, backscan_width, backscan_height,
                                              get_IUSI_FOV=True)
        ...
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(str(path_backscan_video), fourcc, fps,
                              (backscan_width, backscan_height), False)

        with Progress() as progress:
            task = progress.add_task("[green] Backscan conversion in progress...", total=f)
            for i in range(f):
                frame = pre_dsc_image_vectorized(y_cropped[i], dc, rc, theta_c,
                                                 yoffset, xoffset, backscan_width, backscan_height)
                out.write(frame.astype(np.uint8))
                progress.update(task, advance=1)
        out.release()
